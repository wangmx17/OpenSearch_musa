...
try:
    ...
    raise ImportError(
        "The 'fireworks' package is required to use the Fireworks backend. Please install it with: pip install fireworks-ai"
    ) from e
...
class FireworksAgentWorkflowPPOTrainer(AgentWorkflowPPOTrainer):
    ...
    def init_workers(self):
        ...
        ), "Pipeline trainer does not support hybrid engine, assumes Rollout and Actor are not in the different worker group"
        ...
        assert (
            Role.Actor in self.role_worker_mapping
            and Role.Rollout in self.role_worker_mapping
        ), "Actor and Rollout must be in role_worker_mapping"
        ...
        rollout_resource_pool = self.resource_pool_manager.get_resource_pool(
            Role.Rollout
        )
        ...
        self.actor_wg = RayWorkerGroup(
            resource_pool=actor_resource_pool, ray_cls_with_init=actor_cls
        )
        ...
        fireworks_engine = FireworksEngine(
            ...
            sampling_params={
                "temperature": 0.6,
                "top_p": 0.95,
                "max_tokens": self.config.data.max_prompt_length
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer(
                        "rollout_model_update", timing_raw, color="purple"
                    ):
                        checkpoint_folder = (
                        ...
                        fireworks_model_id_prefix = (
                            self.config.fireworks.model_id_prefix
                        )
