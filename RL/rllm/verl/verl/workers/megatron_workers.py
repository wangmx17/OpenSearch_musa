...
_nccl_host_fixed = False


def _ensure_nccl_hostid():
    """Give NCCL a unique host identity so it won't report 'Duplicate GPU'.

    In container environments every node may share the same hostname AND the
    same GPU PCI bus-IDs.  NCCL identifies a GPU by hash(hostname, pci-bus-id),
    so identical hostnames → "Duplicate GPU detected".

    NCCL 2.27.3 does NOT honour the NCCL_HOSTID env-var, so we must actually
    change the process hostname via sethostname(2).

    **Idempotent**: uses a short, fixed-format hostname ``node-{ip}`` so that
    multiple worker processes in the same container all converge to the same
    result instead of appending the IP repeatedly.

    **Must be called BEFORE torch.distributed.init_process_group()** so that
    NCCL reads the new hostname during communicator creation.
    """
    import ctypes

    global _nccl_host_fixed
    if _nccl_host_fixed:
        return
    _nccl_host_fixed = True

    _log = logging.getLogger(__name__)
    old_hostname = socket.gethostname()

    # --- determine a unique IP for this node ---
    node_ip = None
    for cmd in ("hostname -I", "ip -4 addr show bond1 2>/dev/null | awk '/inet /{print $2}' | cut -d/ -f1"):
        try:
            out = subprocess.check_output(cmd, shell=True, text=True).strip().split()[0]
    ...
    if not node_ip:
        _log.warning("[nccl-hostid] Could not determine node IP – skipping hostname fix")
        return

    os.environ["NCCL_HOSTID"] = node_ip

    # --- Build a short, deterministic hostname: "node-{ip}" ---
    # e.g. "node-28-125-152-248" (~20 chars) – well under the 64-char
    # POSIX limit and short enough for Gloo's Unix domain socket paths.
    # This is idempotent: if another worker already called sethostname()
    # in this container, gethostname() already returns the target value.
    unique_hostname = f"node-{node_ip.replace('.', '-')}"

    if old_hostname == unique_hostname:
        _log.info("[nccl-hostid] hostname already correct: %s", old_hostname)
        print(
            f"[nccl-hostid] pid={os.getpid()} hostname already correct: "
            f"{old_hostname} node_ip={node_ip}",
            flush=True,
        )
        return
    ...
    try:
        ...
        buf = unique_hostname.encode()
        rc = libc.sethostname(buf, len(buf))
        new_hostname = socket.gethostname()
        if rc == 0 and new_hostname == unique_hostname:
            _log.info(
                "[nccl-hostid] hostname changed: %s -> %s  (NCCL_HOSTID=%s)",
                old_hostname, new_hostname, node_ip,
            )
        else:
            _log.warning(
                "[nccl-hostid] sethostname returned %d; gethostname()=%s (expected %s)",
                rc, new_hostname, unique_hostname,
            )
    except Exception as e:
        _log.warning("[nccl-hostid] sethostname failed: %s — NCCL may still see duplicate GPUs", e)

    print(
        f"[nccl-hostid] pid={os.getpid()} old_hostname={old_hostname} "
        f"new_hostname={socket.gethostname()} node_ip={node_ip}",
        flush=True,
    )
