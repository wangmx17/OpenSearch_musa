...
def create_custom_optimizer(
    ...
    if use_musa_fused_adamw and optim_name != "adamw_torch":
        ...
        try:
            ...
            logger.warning_rank0(f"Cannot import torch_musa.optim.FusedAdamW: {exc}. Falling back to default optimizer.")
        else:
            decay_parameters = get_parameter_names(model, ALL_LAYERNORM_LAYERS)
            decay_parameters = [name for name in decay_parameters if "bias" not in name]
            optimizer_grouped_parameters = [
                {
                    "params": [
                        param for name, param in model.named_parameters() if name in decay_parameters and param.requires_grad
                    ],
                    "weight_decay": training_args.weight_decay,
                },
                {
                    "params": [
                        param
                        for name, param in model.named_parameters()
                        if name not in decay_parameters and param.requires_grad
                    ],
                    "weight_decay": 0.0,
                },
            ]
            logger.info_rank0("Using torch_musa.optim.FusedAdamW optimizer.")
            return FusedAdamW(
                optimizer_grouped_parameters,
                lr=training_args.learning_rate,
                betas=(training_args.adam_beta1, training_args.adam_beta2),
