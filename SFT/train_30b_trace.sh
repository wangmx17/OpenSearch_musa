...
# ZeRO-init reproduction diagnostics. Disabled by default so production
# training keeps its original behavior and logging volume.
export OPENSEARCH_ZERO_INIT_PROBE="${OPENSEARCH_ZERO_INIT_PROBE:-0}"
export OPENSEARCH_ZERO_INIT_PROBE_HOLD="${OPENSEARCH_ZERO_INIT_PROBE_HOLD:-0}"
export OPENSEARCH_ZERO_INIT_PROBE_HOLD_TIMEOUT_SECONDS="${OPENSEARCH_ZERO_INIT_PROBE_HOLD_TIMEOUT_SECONDS:-1800}"
export OPENSEARCH_ZERO_INIT_PROBE_HEARTBEAT_SECONDS="${OPENSEARCH_ZERO_INIT_PROBE_HEARTBEAT_SECONDS:-10}"
export OPENSEARCH_ZERO_INIT_PROBE_DIR="${OPENSEARCH_ZERO_INIT_PROBE_DIR:-${EXP_LOG_DIR}/zero_init_probe}"
export OPENSEARCH_ZERO_INIT_ATTEMPT_ID="${OPENSEARCH_ZERO_INIT_ATTEMPT_ID:-exp_${EXP_ID}}"
export OPENSEARCH_DIAGNOSTIC_SKIP_FINAL_SAVE="${OPENSEARCH_DIAGNOSTIC_SKIP_FINAL_SAVE:-0}"

case "${OPENSEARCH_ZERO_INIT_PROBE,,}" in
  1|true|yes|on)
    mkdir -p "${OPENSEARCH_ZERO_INIT_PROBE_DIR}/nodes"
    export MCCL_DEBUG_SUBSYS="${MCCL_DEBUG_SUBSYS:-INIT,COLL,NET,ENV}"
    export MCCL_DEBUG_FILE="${MCCL_DEBUG_FILE:-${OPENSEARCH_ZERO_INIT_PROBE_DIR}/mccl_%h_%p.log}"
    export MCCL_SET_THREAD_NAME="${MCCL_SET_THREAD_NAME:-1}"
    ...
    export TORCH_NCCL_DEBUG_INFO_TEMP_FILE="${TORCH_NCCL_DEBUG_INFO_TEMP_FILE:-${OPENSEARCH_ZERO_INIT_PROBE_DIR}/flight_rank_}"
    export TORCH_FR_DUMP_TEMP_FILE="${TORCH_FR_DUMP_TEMP_FILE:-${OPENSEARCH_ZERO_INIT_PROBE_DIR}/flight_rank_}"

    LAUNCHER_PGID="$(ps -o pgid= -p "$$" | tr -d ' ')"
    printf '{"attempt_id":"%s","hostname":"%s","node_rank":%s,"pid":%s,"pgid":%s}\n' \
      "${OPENSEARCH_ZERO_INIT_ATTEMPT_ID}" "${CURRENT_HOSTNAME:-unknown}" "${NODE_RANK}" "$$" "${LAUNCHER_PGID}" \
      > "${OPENSEARCH_ZERO_INIT_PROBE_DIR}/nodes/node_${NODE_RANK}.json"
    ;;
esac
...
echo "       OPENSEARCH_DIAGNOSTIC_SKIP_FINAL_SAVE: ${OPENSEARCH_DIAGNOSTIC_SKIP_FINAL_SAVE}"
echo "       OPENSEARCH_ZERO_INIT_PROBE: ${OPENSEARCH_ZERO_INIT_PROBE}"
echo "       OPENSEARCH_ZERO_INIT_PROBE_HOLD: ${OPENSEARCH_ZERO_INIT_PROBE_HOLD}"
echo "       OPENSEARCH_ZERO_INIT_PROBE_DIR: ${OPENSEARCH_ZERO_INIT_PROBE_DIR}"
...
if [[ "${OPENSEARCH_ZERO_INIT_PROBE,,}" =~ ^(1|true|yes|on)$ ]]; then
  printf '%s\n' "${TRAIN_STATUS}" > "${OPENSEARCH_ZERO_INIT_PROBE_DIR}/nodes/node_${NODE_RANK}.exit_code"
fi
exit "${TRAIN_STATUS}"
