...
HOSTFILE="$1"
TRAIN_SCRIPT="${TRAIN_SCRIPT:-train_30b_test.sh}"
...
for rank in "${!HOSTS[@]}"; do
  ...
  ssh -n -f "$host" \
    "cd '$WORKDIR' && mkdir -p '$LOG_DIR' && nohup setsid env MASTER_ADDR='$MASTER_ADDR' MASTER_PORT='$MASTER_PORT' NNODES='$NNODES' NODE_RANK='$rank' SKIP_INSTALL='$SKIP_INSTALL' bash '$TRAIN_SCRIPT' > '$log_file' 2>&1 < /dev/null &"
done
