...
pick_mccl_ib_device() {
  # Prefer 8x 400G NDR RoCE HCAs on the 172.51 compute fabric.
  # Do NOT use mlx5_bond_0 (25G management bond0 / 10.20.35.x) for MCCL data path.
  # Mapping on this cluster: mlx5_4/5/2/3/10/11/6/9 <-> ens1/2/3/4/5/6/7/8np0
  local names=()
  local name
  for name in mlx5_4 mlx5_5 mlx5_2 mlx5_3 mlx5_10 mlx5_11 mlx5_6 mlx5_9; do
    [[ -d "/sys/class/infiniband/${name}" ]] && names+=("${name}")
  done
  if ((${#names[@]} > 0)); then
    local IFS=,
    echo "${names[*]}"
  ...
  # fallback (legacy / other machines)
...
mkdir -p "${EXP_LOG_DIR}"
NODE_LOG_ID="${NODE_IP:-${CURRENT_HOSTNAME:-unknown}}"
NODE_LOG_ID="${NODE_LOG_ID//[^A-Za-z0-9_.-]/_}"
DEBUG_LOG="${EXP_LOG_DIR}/rank_${NODE_RANK}_${NODE_LOG_ID}.log"
...
#export TORCH_MCCL_TRACE_BUFFER_SIZE="${TORCH_MCCL_TRACE_BUFFER_SIZE:-1048576}"
# ===================== Misc env =====================
export NVTE_FP8=0
...
# ===================== Launch =====================
YAML_CONFIG="${YAML_CONFIG:-${PROJECT_ROOT}/examples/agentic_full/qwen3_vl_full_sft_8b.yaml}"
...
echo "[DEBUG] FORCE_TORCHRUN=${FORCE_TORCHRUN:-<empty>}"
env | grep -E 'FORCE_TORCHRUN|NNODES|NODE_RANK|MASTER_ADDR|MASTER_PORT|RDZV_ID'
echo "[DEBUG] MUSA_LAUNCH_BLOCKING=${MUSA_LAUNCH_BLOCKING:-<unset>}"


export FORCE_TORCHRUN=1
export RDZV_ID="${RDZV_ID:-opensearch-vl-sft}"
# export RDZV_ID="${RDZV_ID:-opensearch-vl-sft-${LOG_TIMESTAMP}}"
